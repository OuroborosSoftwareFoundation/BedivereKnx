...
//                if (StaticDict.NullStringCols.ContainsKey(colName))
//                {
//                    dt.Columns.Add(colName, StaticDict.NullStringCols[colName]); //列格式全部设置为string
//                }
//                else
//                {
//                    dt.Columns.Add(colName, typeof(object)); //列格式全部设置为string
//                }
...
//                        if (cid < dt.Columns.Count) //确保单元格列ID不超过DataTable列ID
//                        {
//                            string cv = GetCellValue(cell, wbp); //单元格内容
//                            if (!string.IsNullOrWhiteSpace(cv)) isAllEmpty = false; //判断行是否全空
//                            if (dt.Columns[cid].DataType == typeof(bool)) //布尔型字符串转换问题
//                            {
//                                dr[cid] = (cv == "1" || cv.ToLower() == "true");
//                            }
//                            else
//                            {
//                                dr[cid] = cv;
